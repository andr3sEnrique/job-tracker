...
import type { StatusCount } from '@jat/shared';
...
/** Horizontal bars read better than a donut for 8 categories. */
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Estado actual de cada candidatura</CardDescription>
      </CardHeader>
      <CardContent>
        {rows ? (
          <ChartContainer config={config} className="aspect-auto h-64 w-full">
            <BarChart data={rows} layout="vertical" margin={{ left: 0, right: 32 }}>
              ...
              <Bar dataKey="count" radius={4}>
                {rows.map((row) => (
                  <Cell key={row.status} fill={STATUS_COLOR_VAR[row.status]} />
                ))}
                <LabelList
                  dataKey="count"
                  position="right"
