...
import { buildGmailSearchQuery } from '../emails/prefilter.js';
...
export class SyncService {
  ...
  async run(userId: string): Promise<SyncResult> {
    ...
    let run = await this.startOrResumeRun(connection);
    try {
   ...
   */
  private async startOrResumeRun(connection: { id: string; initialSyncCompletedAt: Date | null }) {
    const type = connection.initialSyncCompletedAt ? 'MANUAL' : 'INITIAL';
    const unfinished = await this.prisma.syncRun.findFirst({
      where: { connectionId: connection.id, type, status: { not: 'SUCCESS' } },
    ...
    return this.prisma.syncRun.create({ data: { connectionId: connection.id, type } });
  }
